 ...
 * @subpackage APW_Fields
 *
 * @since 1.0
 */

if( ! class_exists( 'APW_Fields' ) ) :

	final class APW_Fields
	{
		...
		{
			$post_types = APW_Utils::get_apw_post_types();
			ob_start();
			?>
		...
		public static function get_orderby_parameters()
		{
			...
			$params = APW_Utils::sanitize_select_array( $params );
		...
		public static function build_field_tax_term( $instance, $widget )
		{
			...
			<?php
			$apw_taxonomies = APW_Utils::get_apw_taxonomies();
		...
		public static function build_field_thumb_size( $instance, $widget )
		{
			...
			<p class="apw-thumb-size-defaults">
				<label for="<?php echo $widget->get_field_id('thumb_size'); ?>">
					<?php _e( 'Choose Registered Image Size:' ); ?>
				</label>
		...
		{
			$sizes = APW_Utils::get_apw_image_sizes( $fields = 'all' );
		...
		public static function build_field_thumb_custom( $instance, $widget )
		{
			...
			<div class="apw-thumb-size-wrap">
				...
				<p>
					<input class="checkbox" type="checkbox" id="<?php echo $widget->get_field_id( 'register_thumb' ); ?>" name="<?php echo $widget->get_field_name( 'register_thumb' ); ?>" <?php checked( $instance['register_thumb'], 1 ); ?>/>
					<label for="<?php echo $widget->get_field_id( 'register_thumb' ); ?>">
						<?php _e( 'Register Custom Size?' ); ?>
					</label>
				</p>
		...
		public static function build_field_excerpt_length( $instance, $widget )
		{
			...
			<p class="apw-excerpt-size-wrap">
				...
				<span class="apw-excerpt-preview">
					<span class="apw-excerpt"><?php echo wp_trim_words( APW_Utils::sample_excerpt(), 15, '&hellip;' ); ?></span>
				</span>
			</p>
